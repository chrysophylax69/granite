...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // granite: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "granite Signed Message:\n";
...
{
    // granite: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // granite: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int GetNextWorkRequired_V3(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // thanks to RealSolid & WDC for the basis of this code, modified by Mammix2 for granite
...
{
    // granite: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc3, 0xb9, 0xde }; // granite: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("graniteMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("graniteMiner : ProcessBlock, block not accepted");
    }
